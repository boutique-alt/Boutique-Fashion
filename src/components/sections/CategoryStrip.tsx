...
export default function CategoryStrip() {
  ...
  return (
    <div className="relative w-full z-20 border-b border-charcoal/10 bg-cream">
      {/* Top Tabs */}
      <div className="mx-auto flex max-w-7xl justify-center gap-8 px-8 pt-3">
        {['WOMEN', 'MEN'].map((tab) => (
          <button
            ...
            } after:absolute after:bottom-0 after:left-1/2 after:h-px after:w-0 after:-translate-x-1/2 after:bg-maroon after:transition-all after:duration-300 ${activeTab === tab ? 'after:w-full' : 'hover:after:w-1/2'}`}
          >
            {tab}
          </button>
        ))}
      ...
      <div className="mx-auto flex max-w-7xl items-start overflow-x-auto px-4 py-4 scrollbar-hide md:justify-center md:px-6">
        ...
        <div className="mx-4 h-12 w-px shrink-0 bg-charcoal/20" />
        ...
        <div className="flex gap-6 md:gap-8">
          {displayedCategories.map((cat) => (
            ...
            >
              ...
              <span className="max-w-[80px] text-center font-sans text-[9px] font-bold uppercase tracking-wider text-charcoal/90 group-hover:text-maroon transition-colors duration-300">
                {cat.title}
              </span>
            </Link>
          ))}
