        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-8">
          {categories.map((cat) => (
            <Link
              key={cat.id}
              to={cat.href}
              className="group flex flex-1 flex-col items-center basis-[40%] md:basis-auto min-w-[140px] max-w-[350px]"
            >
              <div className="relative aspect-square w-full overflow-hidden rounded-full bg-[#f8f4f4] p-4 transition-transform duration-500 group-hover:scale-[1.03]">
                <img
                  src={cat.image}
                  alt={cat.label}
                  loading="lazy"
